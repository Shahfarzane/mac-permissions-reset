#!/usr/bin/env bash
# Kill running instances, package (ad-hoc signed), relaunch, verify.
set -euo pipefail
...
APP_PROCESS_PATTERN="${APP_NAME}.app/Contents/MacOS/${APP_BINARY}"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/.build/debug/${APP_BINARY}"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/.build/release/${APP_BINARY}"
RUN_TESTS=0
RELEASE_ARCHES=""
...
pkill -x "${APP_BINARY}" 2>/dev/null || true
...
if ! open "${APP_BUNDLE}"; then
  ...
  "${APP_BUNDLE}/Contents/MacOS/${APP_BINARY}" >/dev/null 2>&1 &
  disown
fi
